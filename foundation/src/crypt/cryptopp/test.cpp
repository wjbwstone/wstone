...
std::string ECB_AESEncryptStr(std::string sKey, const char* plainText) {
	...
	//ECB_Mode<AES>::Encryption ecbEncryption(aesEncryption);
	ECB_Mode_ExternalCipher::Encryption ecbEncryption(aesEncryption);
	//StreamTransformationFilter ecbEncryptor(ecbEncryption, new HexEncoder(new StringSink(outstr)));
	StreamTransformationFilter ecbEncryptor(ecbEncryption, new Base64Encoder(new StringSink(outstr), false));
...
int scoped_main(int argc, char *argv[])
{
	...
	std::string ev = "111111klsjfklej!";
	auto v = ECB_AESEncryptStr("sdklfjeklj!@#$!", ev.c_str());

	XChaCha20::Encryption cha20E;
	cha20E.SetKeyWithIV()
	...
	encoder.Put((CryptoPP::byte*)v.c_str(), v.size());
	encoder.MessageEnd();
