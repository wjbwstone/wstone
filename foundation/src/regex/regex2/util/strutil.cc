      ...
      default:
        ...
        if (c < ' ' || c > '~') {
          if (dest_len - used < 5)   // space for four-character escape + \0
            return (size_t)-1;
          _snprintf_s(dest + used, 5, 4, "\\%03o", c);
          used += 4;
        } else {
          dest[used++] = c; break;
        }
...
static void StringAppendV(std::string* dst, const char* format, va_list ap) {
  ...
  int result = _vsnprintf_s(space, sizeof(space),
      sizeof(space) -1, format, backup_ap);
  va_end(backup_ap);
  ...
  while (true) {
    ...
    va_copy(backup_ap, ap);
    result = _vsnprintf_s(buf, length, length - 1, format, backup_ap);
    va_end(backup_ap);
